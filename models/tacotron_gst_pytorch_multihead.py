...
class STL(nn.Module):
    def __init__(self, num_gst, style_embed_depth, num_heads, num_units):
        super().__init__()
        self.embed = nn.Parameter(torch.FloatTensor(num_gst, num_units))  # 10 x 128
        init.normal_(self.embed, mean=0, std=0.5)
        self.attention2 = nn.MultiheadAttention(embed_dim=num_units, num_heads=num_heads)
    ...
    def forward(self, x):
        ...
        q1 = query.transpose(1, 0)
        k = keys.transpose(1, 0)
        v = keys.transpose(1, 0)
        style_embeddings, style_embeddings_weights = self.attention2(q1, k, v)
        # style_embeddings, attn_output_weights = self.attention(query, keys, keys)
        # print(attn_output.shape, attn_output_weights.shape)
        # print(style_embeddings.shape, style_embeddings_weights.shape)
        return style_embeddings.transpose(1, 0)
...
class Encoder(nn.Module):
    ...
    def forward(self, x, m):
        ...
        # GST Style attention
        style_embeddings = self.stl(ref_encoder_outputs)  # 8 x 1 x 128 for pytorch multihead
...
class Tacotron(nn.Module):
    def __init__(self, embed_dims, num_chars, encoder_dims, decoder_dims, n_mels, fft_bins, postnet_dims,
        ...
        self.encoder = Encoder(embed_dims, num_chars, encoder_dims,
                               ...
                               ref_enc_filters, n_mels, reference_depth,
                               num_gst, num_heads, style_embed_depth, style_att_dim)
        # normal without anything
        self.encoder_proj = nn.Linear(decoder_dims, decoder_dims, bias=False)
        # for custom multihead
        # self.encoder_proj = nn.Linear(decoder_dims*2, decoder_dims, bias=False)
        # for pytorch multihead
        self.encoder_proj = nn.Linear(decoder_dims + decoder_dims // 2, decoder_dims, bias=False)
